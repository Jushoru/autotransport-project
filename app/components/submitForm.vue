...
import {useRequestFormValidation} from "~/composables/driver/useRequestFormValidation";
import {getCurrentTime, getToday} from "~/utils/currentDate";
import type {requestTableRowType} from "~/types/demoDbTypes";
...
.list-box {
  ...
  &__trigger {
    ...
    text-align: left;
    outline: none;
    color: var(--demo-accent-color);
    line-height: 1.25rem;
    border: 1px solid var(--demo-accent-color);
    display: flex;
    ...
    &:focus-visible {
      border-color: var(--demo-outline-color);
      box-shadow: 0 0 0 2px var(--demo-outline-shadow);
    }
  }
